...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(
        ...
        logger.info(
            "rate limit: %.0f/min, burst %d, comparison costs %d, trusting %d proxy hop(s)",
    ...
    async def dispatch(
        ...
        client = client_key(request, self._settings.trusted_proxy_hops)
...
def client_key(request: Request, trusted_proxy_hops: int) -> str:
    ...
    X-Forwarded-For is a list to which each proxy appends the address it
    received the request from. So the rightmost entry is the peer our nearest
    proxy saw, the one before it is the peer that proxy's upstream saw, and so
    on leftwards. Everything to the left of the trusted hops is whatever the
    original caller chose to send.

    Reading the leftmost entry -- the obvious choice, and what this did
    first -- means a client that sets the header itself gets a different rate
    limit bucket on every request. That does not fail loudly: with one real
    source address a spoofable implementation and a correct one look
    identical, so it has to be reasoned about rather than tested by hitting
    the URL a few times.

    Args:
        request: The incoming request.
        trusted_proxy_hops: How many proxies sit in front of this process.
            Zero ignores the header entirely and uses the socket peer, which is
            correct when nothing is in front of us and the header is therefore
            pure user input.
    """
